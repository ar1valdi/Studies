function [integration_error, Nt, ft_5, integral_1000] = zadanie3()
    ...
    integral_1000 = simp_integral(@(i) calc_malfunction_chance(i), 1000);
    
    for i = 1:length(Nt)
        integration_result(i) = simp_integral(@(i) calc_malfunction_chance(i), Nt(i));
    ...
    xlabel("Chunks num");
    ylabel("Error");
    title("Errors using Simpson's integral");
end