function [integration_error, Nt, ft_5, integral_1000] = zadanie1()
    ...
    integral_1000 = sq_integral(@(i) calc_malfunction_chance(i), 1000);
    
    for i = 1:length(Nt)
        integration_result(i) = sq_integral(@(i) calc_malfunction_chance(i), Nt(i));
    ...
    xlabel("Squares num");
    ylabel("Error");
    title("Errors using square integral");
    idiff = zeros(200,1);
